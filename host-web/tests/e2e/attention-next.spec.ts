...
test.describe("P3 attention loop + NEXT hero button", () => {
  ...
  test("NEXT button appears when the active workspace has a needs-you pane", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      ...
      payload: { type: "status", dir: "", session: "a", title: "A", attention: "needs_permission", activity: "running", following: true, runningCount: 0, unreadCount: 0 },
    });
    await H.probeStatus(page, {
      sourcePaneId: b,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "b", title: "B", attention: "needs_reply", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
  ...
  test("N=2 ranking: needs_permission outranks needs_reply", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      sourcePaneId: reply,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "r", title: "R", attention: "needs_reply", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await page.waitForTimeout(15);
    await H.probeStatus(page, {
      sourcePaneId: perm,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "p", title: "P", attention: "needs_permission", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
  ...
  test("N=2 ranking: older firstNeedsYouAt wins within the same attention level", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      sourcePaneId: older,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "o", title: "O", attention: "needs_reply", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await page.waitForTimeout(20);
    await H.probeStatus(page, {
      sourcePaneId: newer,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "n", title: "N", attention: "needs_reply", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await expect.poll(async () => H.needsYou(page)).toBe(2);
  ...
  test("cross-workspace needy: NEXT activates the target ws + focuses", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      ...
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "w2", title: "W2", attention: "needs_permission", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    ...
    await H.probeStatus(page, {
      ...
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "w1", title: "W1", attention: "needs_reply", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await expect.poll(async () => H.needsYou(page)).toBe(1);
    await expect(page.locator('[data-testid="attention-next"]')).toBeVisible();
  ...
  test("keyboard-open + cross-pane target: NEXT exits keyboard focus-mode then focuses", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      ...
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "n", title: "N", attention: "needs_permission", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await expect.poll(async () => H.needsYou(page)).toBe(1);
  ...
  test("keyboard-open + same-pane target: NEXT keeps keyboard mode", async ({ page }) => {
    ...
    await H.probeStatus(page, {
      sourcePaneId: pane,
      origin: H.MOCK_ORIGIN,
      payload: { type: "status", dir: "", session: "s", title: "S", attention: "needs_permission", activity: "idle", following: true, runningCount: 0, unreadCount: 0 },
    });
    await expect.poll(async () => H.needsYou(page)).toBe(1);
