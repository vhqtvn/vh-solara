...
//         absent. This is the KEY guard for the `!isChild()` term added by the
//         committed fix (ee00bd8): `sub` HAS a message (so
//         `messages().length > 0` is satisfied) and is a normal scroll surface,
//         so without `!isChild()` the Live pill / Latest button would wrongly
//         render inside a session whose composer is disabled and that can't be
//         prompted directly.
//
...
// (3) Child/subagent session (sub): NEITHER control renders. Guards the
//     `!isChild()` term on both gates. `sub` HAS one message (so
//     `messages().length > 0` is true) and is a normal scroll surface, so
//     without `!isChild()` the Live pill or Latest button would render here.
//     This is the regression guard for the committed fix (ee00bd8).
test("sub: both Live pill and Latest button absent for a child/subagent session", async ({ page }) => {
  ...
  // so waiting for it guarantees the !isChild() gate is stably in force before
  // we assert the controls are absent. (subsession.spec.ts asserts this same
  // element carries the "disabled for subagent" notice.)
